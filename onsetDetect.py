...
def computeODF(inputFile, window, M, N, H):
    ...
    #Determinar cuales con los bins que se utilizaran en cada banda
    iLow = []
    iHigh = []
    for k in range(int(N/2)):
        binFreq = k*fs/N
        if binFreq > 0 and binFreq < 3000:
            iLow.append(k)
        if binFreq > 3000 and binFreq < 10000:
            iHigh.append(k)
    
    indexLow = np.array(iLow) 
    indexHigh = np.array(iHigh)
    
    
    #Determinar la ejergia de  cada banda
    framesLowFreq = xmX[:,indexLow[0]:indexLow[-1]+1]
    framesHighFreq = xmX[:,indexHigh[0]:indexHigh[-1]+1]
    
    lowEnergy = sum(np.transpose((abs(10**(framesLowFreq / 20)))**2))
    lowEnergydB = 10*np.log10(lowEnergy)
    
    highEnergy = sum(np.transpose((abs(10**(framesHighFreq / 20)))**2))
    highEnergydB = 10*np.log10(highEnergy)
    ...
    cont = 1
    finalBin = lowEnergydB.size
    while cont < finalBin:
    	difLowOnset = lowEnergydB[cont]-lowEnergydB[cont-1]
    	difHighOnset = highEnergydB[cont]-highEnergydB[cont-1]
    	
    	if difLowOnset > 0:
    		lowOnset.append(difLowOnset)
    	else:
    		lowOnset.append(0)
    	
    	if difHighOnset > 0:
    		highOnset.append(difHighOnset)
    	else:
    		highOnset.append(0)
    	
    	cont = cont + 1
    	
    ODF = np.transpose(np.array([lowOnset,highOnset]))
