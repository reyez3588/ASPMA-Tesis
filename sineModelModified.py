...
'''
Esta es mi version modificada del modelo sinusoidal, en este caso se cambiara el tracking para que
solo tome en cuenta los armonicos de una fundamental definida, tambien se realiza el cambio para que
lo que retorne la funcion analisis este ordenado, primero fundamental y despues las armonicas en orden
ascendente.
'''
...
def armonicOrder(tfreq1, tmag1, tphase1):
	'''
	En esta funcion se recibe el tracking general realizado y contiene todas las frecuencias que cumplen con minSineDur
	ordena en orden ascendente a partir del valor mas bajo del tracking
	...
	#print(tfreq[0])
	orderFreqAux.append(tfreq1[0])
	orderMagAux.append(tmag1[0])
	orderPhaseAux.append(tphase1[0])

	for n in range(tfreq1.size):
		if n == 0:
			continue

		if tfreq1[n] > orderFreqAux[-1]:
			orderFreqAux.append(tfreq1[n])
			orderMagAux.append(tmag1[n])
			orderPhaseAux.append(tphase1[n])
			continue
		if tfreq1[n] < orderFreqAux[0]:
			orderFreqAux.insert(0,tfreq1[n])
			orderMagAux.insert(0,tmag1[n])
			orderPhaseAux.insert(0,tphase1[n])
			continue

		for k in range(len(orderFreqAux)):	#en caso de que no se cumpla ninguna de las anteriores
			if (tfreq1[n] > orderFreqAux[k]) and (tfreq1[n]<orderFreqAux[k+1]):
				orderFreqAux.insert(k+1, tfreq1[n])
				orderMagAux.insert(k+1, tmag1[n])
				orderPhaseAux.insert(k+1, tphase1[n])

	orderFreq = np.array(orderFreqAux)
	orderMag = np.array(orderMagAux)
	orderPhase = np.array(orderPhaseAux)

	return orderFreq, orderMag, orderPhase

def harmonicClean(tone, orderFreq, orderMag, orderPhase, freqDevOffset=100, freqDevSlope = 0.01 ):
	"""
	Esta funcion toma los picos detectados y ordenados, tomara los que sean armonicos de la fundamental "tone"
	y los regresara en el orden adecuado, dejando ceros si no hay armonicos presentes
	:param tone:
	:param orderFreq:
	:param orderMag:
	:param orderPhase:
	:param freqDevOffset:  con 100 obtuve mejores resultados en la sintesis
	:param freqDevSlope:
	:return:
	"""

	#Crear array con todas las armonicas de 'tone' hasta 20 kHz
	harmonicTones = []
	harmonicTones.append(tone)
	cont = 1
	while True:
		if harmonicTones[-1] > 20000:
			break
		harmonicTones.append(tone * (cont + 1))
		cont = cont + 1
	harmonicTones = np.array(harmonicTones) # numpy array que contiene los harmonicos del tono a analizar

	cleanFreq = np.zeros(harmonicTones.size)
	cleanMag = np.zeros(harmonicTones.size)
	cleanPhase = np.zeros(harmonicTones.size)
	for n in range(harmonicTones.size):
		for k in range (orderFreq.size):
			freqDistance = abs(harmonicTones[n] - orderFreq[k])  # measure freq distance
			if freqDistance < (freqDevOffset + freqDevSlope * orderFreq[k]):  # choose track if distance is small
				cleanFreq[n] = orderFreq[k]  # assign
				cleanMag[n] = orderMag[k]
				cleanPhase[n] = orderPhase[k]


	return cleanFreq, cleanMag, cleanPhase
...
def sineModel(x, fs, w, N, t):
	...
	#-----analysis-----             
		...
		# En este punto se realizara el acomodo de las armonicas y se descartaran las frecuencias que no sean armonicas
		# print(tfreq.shape)
		if len(tfreq) > 0:
			tfreq, tmag, tphase = armonicOrder(tfreq, tmag, tphase)  # funcion para ordenar armonicas
			tfreq, tmag, tphase = harmonicClean(440.0, tfreq, tmag, tphase)  # Funcion para limpiar y dejar solo armonicas
...
def sineModelAnal(x, fs, w, N, H, t, maxnSines = 100, minSineDur=.01, freqDevOffset=20, freqDevSlope=0.01):
	...
	while pin<pend:                                         # while input sound pointer is within sound            
		...
		#MODIFICACINES REALIZADAS AL MODELO SINUSOIDAL
		#En este punto se realizara el acomodo de las armonicas y se descartaran las frecuencias que no sean armonicas
		#print(tfreq.shape)
		if len(tfreq) > 0:
			tfreq, tmag, tphase = armonicOrder(tfreq, tmag, tphase)  # funcion para ordenar armonicas
